...
# Combining across tags and studies? How to prevent?
# We could simply make a unique index for tag x study_id and combine across those indices only..
...
class IntervalsFrame(pd.DataFrame):
    ...
    def format(self) -> IntervalsFrame:
        ...
        if len(missing_cols) > 0:
            raise ValueError(
                f"{type(self).__name__} missing required column(s) '{', '.join(missing_cols)}'"
            )
        ...
        labels[numeric_cols] = labels[numeric_cols].astype("float64")
    ...
    def _is_same_type_assert_not_intervals(self, other) -> bool:
        if isinstance(other, IntervalsFrame) and not isinstance(other, type(self)):
            raise ValueError(
    ...
    def __or__(self, other):
        if not self._is_same_type_assert_not_intervals(other):
            return super().__or__(other)
        return (
            pd.concat([self, other], axis=0)
            .sort_values(["start", *self.additional_cols])
            .drop_duplicates()
        )
